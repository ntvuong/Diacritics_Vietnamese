...
class Bokeh_plotter():
    ...
    def general_line(self, ys, xlabel="x", ylabel="y", title="", width=None, height=None, labels=[], fname="", legend_location="top_right", baselines={}, dashes=["solid"], x=[], avgs=0):
        fpath = self.set_output_fname(fname)
        ...
        if len(ys)==0:
            ...
            print("general_line plot got wrong input:")
            print(ys)
        ...
        sources = [ColumnDataSource(data=dict(x=x[:len(y)],
                                              y=y,
