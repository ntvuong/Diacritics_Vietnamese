    ...
    def init_bokeh_figure(self, theme_suffix="", p=None):
        ...
        if p==None:
            if self.bokeh["width"]==None or self.bokeh["height"]==None:
                ...
                p = figure(plot_width = self.bokeh["width"],
...
class GeneralPlotter(Plotter):
    ...
    def export_bokeh(self, theme_suffix=""):
        ...
        sources = [ColumnDataSource(data=dict(x=selet_not_None(self.x[:len(y)],y),
        ...
        if len(self.histogram["Xs"]) > 0:
            for y, color, label in zip(*[self.histogram[k] for k in ["Xs","colors","labels"]]):
                hist, edges = np.histogram(y, density=True, bins=100)
                ...
                if label!=None:
                    p.quad(top="y", bottom=0, left="left", right="right", source=source, fill_color=color, line_color=color, alpha=0.5, legend_label=label)
                else:
                    p.quad(top="y", bottom=0, left="left", right="right", source=source, fill_color=color, line_color=color, alpha=0.5)
        
        if len(self.baselines["values"]) > 0:
            for name, value, color, dash in zip(*[self.baselines[k] for k in ["labels","values","colors","dashes"]]):
                src = ColumnDataSource(data = {"x": [self.x[0]-1,self.x[self.x_len-1]+1],
                                               "y": [value,value],
                                               "maxim": [value,value],
                                               "label": [name,name]})
                if name!=None:
    ...
    # matplotlib #
    ##############
    def export_matplotlib(self, color_settings={"suffix":"", "grid_color":"0.9", "face_color":"white", "line_color": "black", "bg_transparent":False}):
        with plt.style.context((self.matplotlib["style"])):
            ...
            if self.line45_color!=None:
                min_x=min(self.x)
                max_x=max(self.x)
                plt.plot([min_x,max_x],[min_x,max_x], color = self.line45_color, zorder=10)

            if len(self.baselines["values"]) > 0:
                for label, value, color, dash in zip(*[self.baselines[k] for k in ["labels","values","colors","dashes"]]):
                    plt.plot([self.x[0]-1, self.x[self.x_len-1]+1], [value,value],
                              matplotlib_dashes[dash], label=label, color = color, zorder=20)

            for y,dash,color,label,marker in zip(self.Ys,self.dashes,self.colors,self.legend["labels"],self.markers):
                plt.plot(self.x[:len(y)], y, matplotlib_dashes[dash], marker=marker, color=color, label=label, zorder=30)
            
            if len(self.histogram["Xs"]) > 0:
                for y, color, label in zip(*[self.histogram[k] for k in ["Xs","colors","labels"]]):
                    plt.hist(y, density=True, bins=100, alpha = 0.5, color=color, label=label, zorder=10)

            if self.legend["location"]!=None and (len(self.legend["labels"])>0 or len(self.baselines["labels"]) > 0):
    ...
    def export_all(self):
        ...
        if self.heatmap:
            ...
            for i,(x,y,lbl) in enumerate(zip(self.Xs,self.Ys,self.legend["labels"])):
                ...
                #[hx+'{:02X}'.format(int(255*(r+g+b)/2)).lower() for hx,(r,g,b) in zip(cc.CET_L4,rgb)]
...
class ConfMtxPlotter(Plotter):
    ...
    def export_bokeh(self, theme_suffix=""):
        ...
        if self.bokeh["height"] == None:
            self.bokeh["height"] = 100*len(row_labels)
        if self.bokeh["width"] == None:
            self.bokeh["width"] = 100*len(col_labels)+90
        ...
        select = Select(value='Absolute', options=['Absolute', 'Logarithmic', 'Square', 'Square root'])
        args =dict(
            source = source,
            select = select,
            color_bar = color_bar,
