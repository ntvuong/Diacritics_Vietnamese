...
class Endplot_elem():
    def __init__(self, name, y_name, save_dir, legend_loc="bottom_right"):
        self.data = []
        self.name = name
        self.y_name = y_name
    ...
    def save(self, experiment, names):
        params={
            "Ys": self.data,
            "xlabel": 'epoch',
            "ylabel": self.y_name,
            ...
            "markers": None,
            "legend":{
                "location": self.legend_loc, # "top_left", "top_right", "bottom_left", "bottom_right", None
                "labels": names
            },
            "baselines":{
                "labels": list(self.baselines.keys()),
                "values": list(self.baselines.values()),
                "colors": ["grey"], # can be shorter than labels default: ["grey"]
                "dashes": ["dotted"] # can be shorter than labels default: ["dotted"]
            },
        }

        GeneralPlotter(params, neptune_experiment=experiment).export_all()
    ...
    #def update(self, train_acc, dev_acc, loss):
        for benchmark in self.benchmarks:
            for a_t in self.acc_types:
                ...
                self.aggr_dev_accs[str(benchmark)][a_t].data.append(list(dev_acc[str(benchmark)][a_t]))
    ...
    def save(self,experiment=None):
        for benchmark in self.benchmarks:
            for a_t in self.acc_types:
                ...
                self.aggr_dev_accs[str(benchmark)][a_t].save(experiment, self.names)
        
        self.aggr_loss.save(experiment, self.names)
...
class Logger():
    def __init__(self, config_fname, print_to_scrn = True):
        ...
        with open('neptune_cfg_template.yaml') as file: 
    ...
    def save_accs_plot(self, data, labels, benchmark={"deaccent":None}):
        ...
        params={
            ...
            "dirname": self.pic_dir,
            "dashes": self.accuracy_plot_dashes, #"solid", "dashed", "dotted", "dotdash", "dashdot"
            "markers": None,
            "legend":{
        ...
        GeneralPlotter(params, neptune_experiment=self.experiment).export_all()
